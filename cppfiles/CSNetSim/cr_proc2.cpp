...
CR_Proc::CR_Proc(BaseNode* node, VirtualCommProxy* commproxy)
{
	this->node = node;
...
void CR_Proc::change_neighbor(int addr, bool is_youth, double energy)
{
	...
	while(p != NULL){
		if(p->addr == addr){
			p->is_youth = is_youth;
			p->announce_time = BaseNetwork::period_count;
...
void CR_Proc::onUnitTimeOut()
{
	...
	if( !this->check_ch_alive()){
		...
		this->node->next_hop_addr = BaseNetwork::SINK_ADDR;
		
		this->try_to_become_baby();
		if(this->node->CH_addr == this->node->addr){
			double k = rand() / (RAND_MAX + 1.0);
			if( k > (1 - this->node->energy / EnergyModel::E_INIT)){
		...
		if(this->proc_state == this->PROC_INIT){
			if(this->node->members == NULL && this->node->addr == this->node->CH_addr){
				...
				if( k < 1 - this->node->energy / EnergyModel::E_INIT || this->node->energy / EnergyModel::E_INIT < 0.5){
					//printf("%d i am single ch, try_to_become_baby\n", this->node->addr);
					this->try_to_become_baby();
				}
			}
			...
			if(this->is_receive_youth){
				...
				this->unicast_req_new_ch(this->node->CH_addr);
				this->node->members = NULL;
				this->life_state = this->LIFE_BABY;
			}else{
				this->old_timer --;
	...
	while( p!=NULL ){
		...
		if(p->is_youth){
			if(p->d <= BaseNode::CLUSTER_RADIUS){
				s2 += max(0.0, p->announce_energy - ( BaseNetwork::period_count - p->announce_time ) * c);
				maxe = max( maxe, max(0.0, p->announce_energy - ( BaseNetwork::period_count - p->announce_time ) * c) );
				n ++;
			}else{
				s3 += p->announce_energy - ( BaseNetwork::period_count - p->announce_time ) * c;
			}
		}else{
			m ++;
		}
...
int CR_Proc::find_best_next_hop()
{
	...
	double e;
	while( p!=NULL ){
		if(p->is_youth && p->addr != this->node->next_hop_addr){
			if( p_best == NULL ){
				p_best = p;
				e_best = p_best->announce_energy - ( BaseNetwork::period_count - p_best->announce_time ) * c;
			}else{
				e = p->announce_energy - ( BaseNetwork::period_count - p->announce_time ) * c;
				if(e > e_best){
					p_best = p;
					e_best = e;
				}
	...
	if(p_best == NULL){
#ifdef _print_cr_
		printf("error in find_best_next_hop\n");
#endif
		return -1;
	}else{
		return p_best->addr;
...
void CR_Proc::inclustercast_query_cost()
{
	...
	this->clear_scost();
	this->clear_scost_param();
	this->commproxy->inclustercast(this->node->addr, BaseNode::DATA_CTRL_PACKET_SIZE - BaseNode::CTRL_PACKET_SIZE, this->CMD_QUERY_COST, 0, NULL);
}
...
void CR_Proc::set_scost()
{
	...
	while(p1 != NULL){
		...
		p2 = this->scost_param;
		while(p2 != NULL){
			if(this->network->is_cluster_neighbor(p1->addr, p2->addr)){
				sum_cover += pow(lamda, -(p2->coverage));
			}
			p2 = p2->next;
		}
		cost = 1000*(int)(p1->d / (0.1*this->node->CLUSTER_RADIUS) + 1) + 
				100*(int)(p1->e / (0.001*EnergyModel::E_INIT) + 1) + 
...
#endif
	this->commproxy->unicast(this->node->addr, addr, BaseNode::DATA_CTRL_PACKET_SIZE - BaseNode::CTRL_PACKET_SIZE, this->CMD_ASSIGN_YOUTH, 0, NULL);
}
...
void CR_Proc::receive_announce_old(int addr)
{
	...
	}else if(this->node->CH_addr == this->node->addr){
